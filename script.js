...
function doRegister(){
  toast('✅ Account created! Please wait for CEU verification.');
...
function showPage(id){
  ...
  el.innerHTML = r ? r() : `<div class="empty"><div class="empty-icon">🚧</div><div class="empty-title">Page not yet available</div><div class="empty-text">This feature is coming in the next sprint.</div></div>`;
...
student:{
  ...
  vacancies:()=>`
    ...
    <div class="chips">
      <div class="chip on" onclick="filterChip(this)">All</div>
      <div class="chip" onclick="filterChip(this)">IT & Software</div>
      <div class="chip" onclick="filterChip(this)">Engineering</div>
      <div class="chip" onclick="filterChip(this)">Finance</div>
      <div class="chip" onclick="filterChip(this)">Marketing</div>
      <div class="chip" onclick="filterChip(this)">Logistics</div>
    </div>
    ${['MTC Namibia|Junior Software Developer Intern|Windhoek|6 months|15 May 2025|2|IT & Software|Work with the engineering team on mobile app features and REST APIs. React Native experience preferred.|b-green|Open',
       'Bank Windhoek|Data Analyst Intern|Windhoek|4 months|20 May 2025|3|Finance|Analyse customer data, produce dashboards. SQL, Excel and Power BI required.|b-green|Open',
       'Telecom Namibia|Network Operations Intern|Windhoek|6 months|10 May 2025|1|IT & Software|Assist the NOC team with monitoring, incident response and network documentation.|b-amber|Closing soon',
       'Namibia Breweries Ltd|Marketing Coordinator Intern|Windhoek|6 months|01 Jun 2025|2|Marketing|Support brand campaigns, social media and market research for the marketing department.|b-green|Open',
    ].map(r=>{const[co,pos,loc,dur,dead,slots,field,desc,bc,status]=r.split('|');return`
    <div class="vac" onclick="openApplyModal('${co}','${pos}')">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">
        <div class="vac-title">${pos}</div>
        <span class="badge ${bc}" style="flex-shrink:0"><span class="b-dot"></span>${status}</span>
      </div>
      <div class="vac-meta">🏢 ${co} &nbsp;·&nbsp; 📍 ${loc} &nbsp;·&nbsp; ⏱ ${dur} &nbsp;·&nbsp; 📅 Deadline: ${dead} &nbsp;·&nbsp; ${slots} slot${slots>1?'s':''}</div>
      <div class="vac-desc">${desc}</div>
      <div class="vac-footer">
        <button class="btn btn-primary btn-sm" onclick="event.stopPropagation();openApplyModal('${co}','${pos}')">Apply now</button>
        <button class="btn btn-outline btn-sm" onclick="event.stopPropagation();toast('💾 Vacancy saved!')">Save</button>
        <span class="badge b-blue" style="margin-left:auto">${field}</span>
      </div>
    </div>`}).join('')}`,
  ...
  applications:()=>`
    ...
    ${[
      {pos:'IT Support Intern',co:'Namibia Breweries Ltd',applied:'02 Feb 2025',step:4,status:'Placed',bc:'b-green'},
      {pos:'Junior Developer Intern',co:'MTC Namibia',applied:'04 Mar 2025',step:2,status:'Shortlisted',bc:'b-blue'},
      {pos:'Graduate IT Trainee',co:'FNB Namibia',applied:'12 Feb 2025',step:-1,status:'Not successful',bc:'b-red'},
    ].map(a=>`
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:14px">
        <div><div style="font-size:15px;font-weight:700;color:var(--navy)">${a.pos}</div><div style="font-size:12px;color:var(--muted);margin-top:2px">${a.co} &nbsp;·&nbsp; Applied ${a.applied}</div></div>
        <span class="badge ${a.bc}" style="flex-shrink:0"><span class="b-dot"></span>${a.status}</span>
      </div>
      ${a.step>0?`
        ...
        <div style="display:flex;gap:0">
          ...
          <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:5px;position:relative;z-index:1">
            <div style="width:26px;height:26px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;border:2px solid var(--white);
              background:${i<a.step?'var(--success)':i===a.step-1?'var(--accent)':'var(--border2)'};">
              ${i<a.step?'✓':i===a.step-1?'●':''}
            </div>
            <div style="font-size:10px;font-weight:600;color:${i<=a.step-1?'var(--text2)':'var(--muted)'};text-align:center;line-height:1.3">${s}</div>
          </div>`).join('')}
        </div>
        <div style="position:absolute;top:13px;left:0;right:0;height:2px;background:var(--border);z-index:0"></div>
        <div style="position:absolute;top:13px;left:0;height:2px;background:var(--accent);z-index:0;width:${(a.step-1)/4*100}%"></div>
      ...
      <div class="btn-group" style="margin-top:14px">
        ${a.step===4?`<button class="btn btn-primary btn-sm" onclick="showPage('placement')">View placement →</button>`:''}
        ${a.step===2?`<button class="btn btn-accent btn-sm" onclick="toast('🎉 Reviewing offer from MTC Namibia…')">Review offer</button>`:''}
        <button class="btn btn-ghost btn-sm" onclick="toast('📄 Application details opened.')">Details</button>
        ${a.step<0?`<button class="btn btn-outline btn-sm" onclick="showPage('vacancies')">Browse more vacancies</button>`:''}
      </div>
    </div>`).join('')}`,
...
employer:{
  ...
  vaclist:()=>`
    ...
    <div class="card">
      <div class="tbl-wrap"><table>
        <thead><tr><th>Position</th><th>Slots</th><th>Deadline</th><th>Applicants</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
          <tr><td><strong>IT Support Intern</strong></td><td>2</td><td>28 Feb 2025</td><td>8</td><td><span class="badge b-gray">Closed</span></td><td><div class="btn-group"><button class="btn btn-outline btn-sm" onclick="showPage('applicants')">Applicants</button><button class="btn btn-ghost btn-sm" onclick="toast('✏️ Editing vacancy…')">Edit</button></div></td></tr>
          <tr><td><strong>Systems Admin Intern</strong></td><td>1</td><td>30 May 2025</td><td>3</td><td><span class="badge b-green"><span class="b-dot"></span>Open</span></td><td><div class="btn-group"><button class="btn btn-outline btn-sm" onclick="showPage('applicants')">Applicants</button><button class="btn btn-ghost btn-sm" onclick="toast('✏️ Editing vacancy…')">Edit</button><button class="btn btn-ghost btn-sm" onclick="confirmDelete('vacancy')">🗑</button></div></td></tr>
        </tbody>
      </table></div>
    </div>`,
  ...
  postvac:()=>`
    ...
    <div class="card">
      <div class="f-row">
        <div class="f-group"><label class="f-label">Position title *</label><input class="f-input" placeholder="e.g. IT Support Intern"></div>
        <div class="f-group"><label class="f-label">Department *</label><input class="f-input" placeholder="e.g. Information Technology"></div>
      </div>
      <div class="f-row" style="margin-top:10px">
        <div class="f-group"><label class="f-label">Duration (months) *</label><input class="f-input" type="number" placeholder="6" min="1" max="12"></div>
        <div class="f-group"><label class="f-label">Available slots *</label><input class="f-input" type="number" placeholder="2" min="1"></div>
      </div>
      <div class="f-row" style="margin-top:10px">
        <div class="f-group"><label class="f-label">Application deadline *</label><input class="f-input" type="date"></div>
        <div class="f-group"><label class="f-label">Location *</label><input class="f-input" value="Windhoek, Namibia"></div>
      </div>
      <div class="f-row" style="margin-top:10px">
        <div class="f-group">
          <label class="f-label">Field / sector *</label>
          <select class="f-select"><option>IT & Software</option><option>Engineering</option><option>Finance</option><option>Marketing</option><option>Logistics</option><option>Other</option></select>
        </div>
        <div class="f-group"><label class="f-label">Remuneration</label><input class="f-input" placeholder="e.g. NAD 3,000/month or Unpaid"></div>
      </div>
      <div class="f-group" style="margin-top:10px">
        <label class="f-label">Required programme(s)</label>
        <input class="f-input" placeholder="e.g. BSc Computer Science, BSc IT">
      </div>
      <div class="f-group" style="margin-top:10px">
        <label class="f-label">Role description & requirements *</label>
        <textarea class="f-textarea" style="height:120px" placeholder="Describe the role, responsibilities, and required skills…"></textarea>
      </div>
      <div style="margin-top:16px;padding:12px 14px;background:var(--accent-dim);border-radius:var(--r-sm);font-size:12px;color:var(--text2)">
        ℹ️ Your vacancy will be reviewed by the CEU before being published. This typically takes 1–2 business days.
      </div>
      <div class="btn-group" style="margin-top:14px">
        <button class="btn btn-primary" onclick="toast('✅ Vacancy submitted to CEU for review.');showPage('vaclist')">Submit for review</button>
  ...
  applicants:()=>`
    <div class="ph"><div class="ph-left"><div class="ph-title">Applicants</div><div class="ph-sub">IT Support Intern — Namibia Breweries Ltd</div></div></div>
    <div class="card">
      <div class="tbl-wrap"><table>
        <thead><tr><th>Student</th><th>Programme</th><th>Year</th><th>Applied</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
          <tr><td><strong>Tjihezu Tjihozu</strong></td><td>BSc Computer Science</td><td>3rd</td><td>02 Mar 2025</td><td><span class="badge b-green">Accepted</span></td><td><div class="btn-group"><button class="btn btn-outline btn-sm" onclick="toast('📄 CV opened')">View CV</button><button class="btn btn-ghost btn-sm" onclick="toast('👤 Profile viewed')">Profile</button></div></td></tr>
          <tr><td><strong>Dyrall Beukes</strong></td><td>BEng Electrical</td><td>3rd</td><td>04 Mar 2025</td><td><span class="badge b-amber">Pending</span></td><td><div class="btn-group"><button class="btn btn-success btn-sm" onclick="toast('✅ Application accepted! CEU will be notified.')">Accept</button><button class="btn btn-outline btn-sm" onclick="toast('⏩ Student shortlisted.')">Shortlist</button><button class="btn btn-danger btn-sm" onclick="toast('❌ Application declined.')">Decline</button></div></td></tr>
          <tr><td><strong>Anselmo Martins</strong></td><td>BSc IT</td><td>2nd</td><td>06 Mar 2025</td><td><span class="badge b-red">Not selected</span></td><td><div class="btn-group"><button class="btn btn-outline btn-sm" onclick="toast('📄 CV opened')">View CV</button></div></td></tr>
        </tbody>
      </table></div>
    </div>`,
  ...
  evaluate:()=>`
    ...
    <div class="card">
      <div class="f-row">
        ...
        <div class="f-group"><label class="f-label">Evaluation type *</label><select class="f-select"><option>Mid-term evaluation</option><option>Final evaluation</option></select></div>
      </div>
      <div style="margin-top:14px;margin-bottom:6px;font-size:12px;font-weight:700;color:var(--text2)">Performance scores (0–100)</div>
      <div class="f-row">
        <div class="f-group"><label class="f-label">Technical performance</label><input class="f-input" type="number" value="82" min="0" max="100"></div>
        <div class="f-group"><label class="f-label">Work ethic & attitude</label><input class="f-input" type="number" value="88" min="0" max="100"></div>
        <div class="f-group"><label class="f-label">Communication skills</label><input class="f-input" type="number" value="76" min="0" max="100"></div>
        <div class="f-group"><label class="f-label">Teamwork & initiative</label><input class="f-input" type="number" value="80" min="0" max="100"></div>
      </div>
      <div class="f-group" style="margin-top:10px">
        <label class="f-label">Overall recommendation</label>
        <select class="f-select"><option>Highly recommended for employment</option><option>Recommended</option><option>Satisfactory performance</option><option>Needs improvement</option></select>
      </div>
      <div class="f-group" style="margin-top:10px">
        <label class="f-label">Detailed comments *</label>
        <textarea class="f-textarea" style="height:120px" placeholder="Provide a thorough assessment of the intern's performance…">The intern demonstrated strong initiative and a proactive attitude. Technical skills are solid for the level of study. Would consider for a graduate position.</textarea>
      </div>
      <div class="btn-group" style="margin-top:16px">
        <button class="btn btn-primary" onclick="toast('⭐ Evaluation submitted to CEU and lecturer.');showPage('interns')">Submit evaluation</button>
        <button class="btn btn-outline" onclick="toast('💾 Draft saved.')">Save draft</button>
      </div>
    </div>`,
},
...
ceu:{
  ...
  approvals:()=>`
    <div class="ph"><div class="ph-left"><div class="ph-title">Pending approvals</div><div class="ph-sub">7 placements require CEU review and sign-off</div></div></div>
    <div class="card">
      <div class="tbl-wrap"><table>
        <thead><tr><th>Student</th><th>Programme</th><th>Employer</th><th>Role</th><th>Submitted</th><th>Documents</th><th>Actions</th></tr></thead>
        <tbody>
          <tr><td><strong>Dyrall Beukes</strong></td><td>BEng Electrical</td><td>MTC Namibia</td><td>Network Intern</td><td>04 Mar 2025</td><td><span class="badge b-green">Complete</span></td><td><div class="btn-group"><button class="btn btn-success btn-sm" onclick="toast('✅ Dyrall approved! Student & employer notified.')">Approve</button><button class="btn btn-danger btn-sm" onclick="toast('❌ Rejected. Student notified.')">Reject</button><button class="btn btn-ghost btn-sm" onclick="toast('📄 Application details opened.')">Details</button></div></td></tr>
          <tr><td><strong>Pascal Tandula</strong></td><td>BSc IT</td><td>Bank Windhoek</td><td>Data Analyst</td><td>05 Mar 2025</td><td><span class="badge b-green">Complete</span></td><td><div class="btn-group"><button class="btn btn-success btn-sm" onclick="toast('✅ Pascal approved!')">Approve</button><button class="btn btn-danger btn-sm" onclick="toast('❌ Rejected.')">Reject</button><button class="btn btn-ghost btn-sm">Details</button></div></td></tr>
          <tr><td><strong>Silvio Ivano</strong></td><td>BSc CS</td><td>Telecom Namibia</td><td>NOC Intern</td><td>06 Mar 2025</td><td><span class="badge b-amber">Missing CV</span></td><td><div class="btn-group"><button class="btn btn-outline btn-sm" onclick="toast('📧 Reminder sent to Silvio.')">Request docs</button><button class="btn btn-danger btn-sm" onclick="toast('❌ Rejected.')">Reject</button></div></td></tr>
        </tbody>
      </table></div>
    </div>`,
...
/* ══════════════════════════════════════
   MODALS
══════════════════════════════════════ */
function openApplyModal(company, position){
  openModal({title:`Apply — ${position}`,sub:company,body:`
    <div class="f-row">
      <div class="f-group"><label class="f-label">Full name</label><input class="f-input" value="${USERS.student.name}"></div>
      <div class="f-group"><label class="f-label">Student number</label><input class="f-input" value="${USERS.student.studentNo}"></div>
    </div>
    <div class="f-group" style="margin-top:10px"><label class="f-label">Programme</label><input class="f-input" value="${USERS.student.programme}" readonly></div>
    <div class="f-group" style="margin-top:10px">
      <label class="f-label">Cover letter *</label>
      <textarea class="f-textarea" style="height:110px" placeholder="Briefly describe your motivation and relevant experience…"></textarea>
    </div>
    <div class="f-row" style="margin-top:10px">
      <div class="f-group"><label class="f-label">CV attached</label><input class="f-input" value="Tjihozu_CV_2025.pdf" readonly></div>
      <div class="f-group"><label class="f-label">Available from *</label><input class="f-input" type="date" value="2025-07-01"></div>
    </div>`,
    actions:`<button class="btn btn-outline" onclick="closeModal()">Cancel</button>
...
function submitApplication(){
  closeModal();
  toast('🎉 Application submitted! CEU will review and notify you.');
  setTimeout(()=>showPage('applications'),700);
}

function openLogModal(){
  const today = new Date().toISOString().split('T')[0];
  openModal({title:'New logbook entry',sub:'Namibia Breweries Ltd — Week 15',body:`
    <div class="f-row">
      <div class="f-group"><label class="f-label">Week number</label><input class="f-input" value="15"></div>
      <div class="f-group"><label class="f-label">Date</label><input class="f-input" type="date" value="${today}"></div>
    </div>
    <div class="f-row" style="margin-top:10px">
      <div class="f-group"><label class="f-label">Hours worked</label><input class="f-input" type="number" placeholder="8" min="1" max="12"></div>
      <div class="f-group"><label class="f-label">Activity type</label><select class="f-select"><option>Technical work</option><option>Training / shadowing</option><option>Meetings</option><option>Documentation</option><option>Other</option></select></div>
    </div>
    <div class="f-group" style="margin-top:10px">
      <label class="f-label">Entry title</label>
      <input class="f-input" placeholder="e.g. Server maintenance and documentation">
    </div>
    <div class="f-group" style="margin-top:10px">
      <label class="f-label">Detailed activities *</label>
      <textarea class="f-textarea" style="height:110px" placeholder="Describe what you worked on in detail. Be specific about tasks, tools used, and learning outcomes…"></textarea>
    </div>`,
    actions:`<button class="btn btn-outline" onclick="closeModal()">Cancel</button>
             <button class="btn btn-primary" onclick="closeModal();toast('📓 Logbook entry saved! Sent to lecturer for approval.')">Save entry</button>`
  });
}

function openUploadModal(){
  openModal({title:'Upload document',body:`
    <div class="f-group">
...
function openCreateAccountModal(){
  openModal({title:'Create user account',body:`
    <div class="f-row">
